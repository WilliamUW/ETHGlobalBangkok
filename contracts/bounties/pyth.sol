...
///
/// The RandomReward contract implements the IEntropyConsumer interface imported from the Solidity SDK.
/// The interface helps in integrating with Entropy correctly.
contract RandomReward is IEntropyConsumer {
    ...
    event FlipResult(uint64 sequenceNumber, uint256 result);
    ...
    function requestFlip(bytes32 userRandomNumber) external payable {
        ...
        if (msg.value < fee) {
            revert RandomRewardErrors.InsufficientFee();
        }
    ...
    function entropyCallback(
        ...
        emit FlipResult(sequenceNumber, uint256(randomNumber) % 5);
    }
